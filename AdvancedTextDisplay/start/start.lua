...
function _G.loadOptionFile()
	...
	--Loads the file
	local file = fs.open("/AdvancedTextDisplay/config/options.txt","r")
...
function _G.saveOptionFile()
	...
	local file = fs.open("/AdvancedTextDisplay/config/options.txt","w")
    file.writeLine(list)
	file.close()
	print("Saved.")
end
...
function _G.checkUpdates()
	...
	local success, ErrorStatement = pcall(downloadFile, repoUrl..currBranch.."/AdvancedTextDisplay/",currBranch..".ver")
	local tries = 1
	...
	while not success do
		...
		if tries < 10 then 
			...
			os.sleep(5)
			success, ErrorStatement = pcall(downloadFile, repoUrl..currBranch.."/AdvancedTextDisplay/",currBranch..".ver")
		else 
	...
	--downloadFile(repoUrl..currBranch.."/AdvancedTextDisplay/",currBranch..".ver")
...
--
	...
	if autoUpdate == 1 then
		shell.run("/AdvancedTextDisplay/install/installer.lua update "..branch)
		os.reboot()
		return
	end
...
function initClasses()
    -- Create base paths
    local binPath = "/AdvancedTextDisplay/classes/"
...
shell.run("/AdvancedTextDisplay/program/monitor.lua")

shell.completeProgram("/AdvancedTextDisplay/start/start.lua")
