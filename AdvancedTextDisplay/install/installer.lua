...
function readConfigFile()
  local fileRead = fs.open("/AdvancedTextDisplay/config/options.txt","r")
...
function updateConfigFile(oldConfig)
  ...
  local fileSave = fs.open("/AdvancedTextDisplay/config/options.txt","w")
...
function getVersion()
  ...
  if branch == "main" then
    writeFile("main.ver")
    fileData = fs.open("/AdvancedTextDisplay/main.ver","r")
  elseif branch == "development" then
    writeFile("development.ver")
    fileData = fs.open("/AdvancedTextDisplay/development.ver","r")
  end
...
if not update then
  ...
  while out2 do
    ...
    if selectedLang:yesCheck(input) then
      ...
      file.writeLine("shell.run(\"/AdvancedTextDisplay/start/start.lua\")")
      file.close()
      print()
...
--Removes old files
if fs.exists("/AdvancedTextDisplay/program/") then
  shell.run("rm /AdvancedTextDisplay/")
end
...
shell.completeProgram("/AdvancedTextDisplay/install/installer.lua")
